...
// Geoapify calls.
const buildParkCandidates = async (origin, areaFilter = null) => {
  const parkFeatures = await searchParksGeoapify(origin.lat, origin.lon, 15000, areaFilter);
  const trailFeatures = await searchTrailsGeoapify(origin.lat, origin.lon, 15000, areaFilter);
...
const FEATURE_SEARCH_RADIUS_M = 2000;

// Scores/reorders parks by how many requested features are found nearby.
// Batches every requested feature into ONE Geoapify query per park (not one
// query per feature per park) — keeps the extra call count at parks.length
// rather than parks.length * features.length.
const scoreParksByFeatures = async (parks, features) => {
  ...
  const scored = await Promise.all(parks.map(async (park) => {
    try {
      ...
      const foundCategories = new Set(found.flatMap((f) => f.properties.categories || []));
      const foundNames = found.map((f) => (f.properties.name || '').toLowerCase());

      const matchedFeatures = requested.filter((keyword) => {
        const inCategory = FEATURE_CATEGORY_MAP[keyword].split(',').some((cat) => foundCategories.has(cat));
        if (!inCategory) return false;
        const hints = FEATURE_NAME_HINTS[keyword];
        return hints ? foundNames.some((name) => hints.some((hint) => name.includes(hint))) : true;
      });
      return { ...park, matchedFeatures };
    } catch (err) {
      return { ...park, matchedFeatures: [] };
    }
  }));

  return scored.sort((a, b) => b.matchedFeatures.length - a.matchedFeatures.length);
};
...
const performParkSearch = async ({ location, date, difficulty, length, elevation, activity, features }) => {
  ...
  let parks = cached.allParks.filter(matchesFilters).slice(0, 8);
...
// so the caller can log a useful reason per attempt.
const fetchOverpassOnce = async (url, query) => {
  ...
  try {
    ...
    if (err.name === 'AbortError') throw new Error(`timed out after ${OVERPASS_TIMEOUT_MS / 1000}s`);
    throw err;
  } finally {
...
const fetchOverpassWithRetry = async (query) => {
...
const extractBoundaryCandidates = (overpassData) => {
  return (overpassData.elements || [])
    ...
    .filter((polygon) => polygonArea(polygon) > 0);
};
...
const BOUNDARY_SEARCH_RADII_M = [200, 500, 1500];
...
// One Overpass call per radius, unioning every candidate tag — batching
// avoids issuing 5x sequential requests per attempt against a public
// instance that already throttles multi-clause queries (see
// fetchOverpassWithRetry above).
const buildBoundaryQuery = (radius, lat, lon) => {
  const clauses = BOUNDARY_TAGS.map((tag) => `way${tag}(around:${radius},${lat},${lon});`).join('');
  return `[out:json];(${clauses});(._;>;);out geom;`;
};

const fetchParkBoundary = async (lat, lon) => {
  for (const radius of BOUNDARY_SEARCH_RADII_M) {
    const data = await fetchOverpassWithRetry(buildBoundaryQuery(radius, lat, lon));
...
const fetchTrailWaysInBBox = async (bbox, bboxPaddingFraction) => {
  const { south, west, north, east } = padBBox(bbox, bboxPaddingFraction);
  const b = `${south},${west},${north},${east}`;
  const query = `[out:json];(way["highway"="path"]["foot"!="no"]["access"!="private"](${b});way["highway"="footway"]["footway"!="sidewalk"]["footway"!="crossing"]["foot"!="no"]["access"!="private"](${b});way["highway"="track"]["foot"!="no"]["access"!="private"]["motor_vehicle"!="yes"](${b});way["route"="hiking"](${b});way["highway"="path"]["sac_scale"](${b}););(._;>;);out geom;`;
...
const buildParkTrailData = async (lat, lon) => {
  ...
  const [boundedWays, amenities] = await Promise.all([
    fetchTrailWaysInBBox(bbox, config.bboxPaddingFraction),
...
const buildParkLoopData = async (trailData, parkingLat = null, parkingLon = null, filters = {}) => {
  ...
  if (ways.length === 0) {
    return { route: null, rawTrails: [], rawTrailColor: null, message: 'No trail data found for this park' };
  }
  ...
  const rawRoute = findOptimalParkRoute(ways, filters, startNode);
  ...
  return { route, rawTrails: [], rawTrailColor: null, message: null };
};
...
const getParkLoops = async (req, res) => {
  try {
    ...
    const { route, rawTrails, rawTrailColor, message } = cached;
    const { parkingLots, restrooms, parkSize, parkAreaKm2 } = trailData;
    const restroomCount = restrooms.length;

    res.json({ route, rawTrails, rawTrailColor, message, parkingLots, restrooms, restroomCount, parkSize, parkAreaKm2 });
  } catch (error) {
    console.error(error);
